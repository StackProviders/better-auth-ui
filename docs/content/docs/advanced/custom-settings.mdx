...
The default authentication components provided by `better-auth-ui` include built-in settings pages accessible under the same base path as your auth views (e.g., `/auth/settings`, `/auth/security`, etc.).
...
```tsx title="app/dashboard/settings/page.tsx"
import { RedirectToSignIn, SignedIn, SettingsCards } from "@/components/auth";
...
- [Managing Organizations](/advanced/organizations)
- [API Key Management](/advanced/api-keys)
- [Custom User Fields](/advanced/additional-fields)
